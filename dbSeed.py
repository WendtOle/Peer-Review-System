...
def init():
    ...
    user02 = models.User(email='ole-wendt@freenet.de',
                         password=app.bcrypt.generate_password_hash('ole-wendt').decode('utf-8'), role=models.UserRole.REVIEWER)
    user03 = models.User(email='timurOezer@web.de',
                         password=app.bcrypt.generate_password_hash('timurOezer').decode('utf-8'), role=models.UserRole.REVIEWER)
